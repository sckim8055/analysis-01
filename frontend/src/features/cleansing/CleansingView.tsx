  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', padding: '24px' }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', gap: '24px' }}>
        ...
        {/* 이상치/결측치 탭 */}
        {activeTab === 'anomalies' && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', backgroundColor: 'var(--bg-base)', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
                      ...
                      {table.getHeaderGroups().map(headerGroup => (
                        <tr key={headerGroup.id}>
                          {headerGroup.headers.map(header => (
                            <th key={header.id} style={{ padding: '12px', fontWeight: '500', color: 'var(--text-secondary)', borderRight: '1px solid var(--border-color)', width: header.getSize(), minWidth: header.getSize(), whiteSpace: 'nowrap' }}>
