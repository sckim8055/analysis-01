...
export const TTestAnovaView: React.FC = () => {
    ...
    Object.entries(mappedVars).forEach(([role, vars]) => {
      ...
      vars.forEach(v => {
        ...
        if (v.subFactors && v.subFactors.length > 0) {
            v.subFactors.forEach(sf => {
                const finalCols = (sf.itemIds || []).filter(id => survivedMap[id]);
  ...
  useEffect(() => {
    if (factorsPayloadMemo.length > 0 && demographicColumns && demographicColumns.length > 0) {
        handleAnalyze();
    }
  }, []); // Only run once on mount

  const handleAnalyze = async () => {
    if (factorsPayloadMemo.length === 0 || !demographicColumns || demographicColumns.length === 0) return;
      ...
      results.diff_results.forEach((dr: any) => {
        ...
        factors.forEach((fname: string) => {
            if (dr.factors[fname]) {
                ...
                const content = `${stat}${stars}(${p < 0.001 ? '.000' : p.toFixed(3).replace(/^0\./, '.')})`;
                                    ...
                                    <React.Fragment key={dIdx}>
                                        ...
                                        <tr style={{ borderBottom: '1px solid var(--text-primary)' }}>
                                            ...
                                            {factors.map((f: string) => {
                                                ...
                                                const content = `${fres.statistic.toFixed(3)}${stars}(${pStr})`;
