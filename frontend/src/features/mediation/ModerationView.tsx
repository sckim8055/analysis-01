...
export const ModerationView: React.FC = () => {
    ...
    const handleExportMediation = async (dvRes: any, models: any[]) => {
        ...
        try {
            const title = `<표> 독립변수와 ${dvRes.dv_name}의 관계에서 조절변수(${dvRes.mod_name})의 조절효과`;
            const filename = `moderation_table_${dvRes.dv_name}.xlsx`;
            exportHtmlTableToExcel(title, filename, ['mod-table']);
        } catch (err) {
            console.error(err);
            alert('엑셀 다운로드 중 오류가 발생했습니다.');
        } finally {
