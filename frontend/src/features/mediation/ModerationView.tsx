...
export const ModerationView: React.FC = () => {
    ...
    const tabs = React.useMemo(() => {
        ...
        if (savedModelEdges && savedModelEdges.length > 0 && savedModelNodes && savedModelNodes.length > 0) {
                            ...
                            if (inP.isMod || outP.isMod) {
                                ...
                                if (modVar) {
                                    pairs.push({
                                        ...
                                        id: hLabel,
                                        name: `[${hLabel} 조절된 매개] ${ivVar.name} → ${medVar.name} → ${dvVar.name} (조절: ${modVar.name})`,
            ...
            if (pairs.length > 0) return pairs.filter(p => p.analysisType === 'moderation');
        }
        ...
        if (mappedVars.iv && mappedVars.dv) {
            ...
            if (mappedVars.med && mappedVars.med.length > 0 && mappedVars.mod && mappedVars.mod.length > 0) {
                mappedVars.iv.forEach((iv: any) => { mappedVars.med.forEach((med: any) => { mappedVars.mod.forEach((mod: any) => { mappedVars.dv.forEach((dv: any) => { pairs.push({ iv, med, mod, dv, id: `H${counter}`, name: `[H${counter}] ${iv.name} → ${med.name} → ${dv.name} (조절: ${mod.name})`, analysisType: 'moderated_mediation' }); counter++; }); }); }); });
            }
        }
        return pairs.filter(p => p.analysisType === 'moderation');
    ...
    const handleExportHypothesis = async () => {
        ...
        try {
            ...
            results.results.forEach((dvRes: any) => {
                const intCoef = dvRes.model3.coefficients.find((c: any) => c.name.includes('X') || c.name.includes('x'));
                let status = '기각';
                ...
                hypothesesRows.push({
                    '가설': `${hLabel}-${subHIdx++}`,
                    '가설 내용': `'${results.modParentName}'은(는) '${results.ivParentName}'와(과) '${dvRes.dv_name}' 간의 관계를 조절할 것이다.`,
                    '채택여부': status
                });
            });
            ...
            hypothesesRows.unshift({
                '가설': hLabel,
                '가설 내용': `${results.modParentName}은(는) ${results.ivParentName}과(와) ${results.dvParentName} 사이의 관계를 조절할 것이다.`,
                '채택여부': mainStatus
            });
    ...
    const handleExportMediation = async (dvRes: any, models: any[]) => {
        ...
        try {
            ...
            m3.coefficients.forEach((c: any) => {
                ...
                rows.push({
                    '구분': c.name,
                    '모델 I_β': c1.B !== null ? c1.B.toFixed(3) : '',
                    '모델 I_t': c1.t !== null ? c1.t.toFixed(3) : '',
                    '모델 I_p': c1.p !== null ? (c1.p < 0.001 ? '.000' : c1.p.toFixed(3)) : '',
                    '모델 II_β': c2.B !== null ? c2.B.toFixed(3) : '',
                    '모델 II_t': c2.t !== null ? c2.t.toFixed(3) : '',
                    '모델 II_p': c2.p !== null ? (c2.p < 0.001 ? '.000' : c2.p.toFixed(3)) : '',
                    '모델 III_β': c.B !== null ? c.B.toFixed(3) : '',
                    '모델 III_t': c.t !== null ? c.t.toFixed(3) : '',
                    '모델 III_p': c.p !== null ? (c.p < 0.001 ? '.000' : c.p.toFixed(3)) : ''
                });
            });

            rows.push({
                '구분': 'F-value',
                '모델 I_β': m1.f_value !== null ? m1.f_value.toFixed(3) : '', '모델 I_t': '', '모델 I_p': '',
                '모델 II_β': m2.f_value !== null ? m2.f_value.toFixed(3) : '', '모델 II_t': '', '모델 II_p': '',
                '모델 III_β': m3.f_value !== null ? m3.f_value.toFixed(3) : '', '모델 III_t': '', '모델 III_p': ''
            });
            ...
            rows.push({
                '구분': 'R²',
                '모델 I_β': m1.r_squared !== null ? m1.r_squared.toFixed(3) : '', '모델 I_t': '', '모델 I_p': '',
                '모델 II_β': m2.r_squared !== null ? m2.r_squared.toFixed(3) : '', '모델 II_t': '', '모델 II_p': '',
                '모델 III_β': m3.r_squared !== null ? m3.r_squared.toFixed(3) : '', '모델 III_t': '', '모델 III_p': ''
            });

            rows.push({
                '구분': 'Change of R²',
                '모델 I_β': '', '모델 I_t': '', '모델 I_p': '',
                '모델 II_β': m2.r2_change !== null && m2.r2_change !== undefined ? m2.r2_change.toFixed(3) : '', '모델 II_t': '', '모델 II_p': '',
                '모델 III_β': m3.r2_change !== null && m3.r2_change !== undefined ? m3.r2_change.toFixed(3) : '', '모델 III_t': '', '모델 III_p': ''
            });
            ...
            const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/analysis/mediation/export`, {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    title: `<표> 독립변수와 ${dvRes.dv_name} 간의 ${dvRes.mod_name}의 조절효과`,
                    rows: rows
                })
            });
            if (!res.ok) throw new Error('Export failed');
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `moderation_table_${dvRes.dv_name}.xlsx`;
            document.body.appendChild(a);
            a.click();
            a.remove();
        } catch (err) {
    ...
    const generateInterpretation = () => {
        if (!results) return "";
        let text = `본 연구에서 설정한 가설 ${results.hypothesisName}을(를) 검증하기 위하여, 독립변수인 [${results.ivParentName}]이(가) 종속변수인 [${results.dvParentName}]에 미치는 영향에서 조절변수인 [${results.modParentName}]의 조절효과를 분석하였다. 이를 위해 위계적 다중회귀분석(Hierarchical Multiple Regression Analysis)을 실시하였다.

`;

        results.results.forEach((dvRes: any) => {
            text += `### 📌 종속변수: [${dvRes.dv_name}]에 대한 조절효과 검증

`;
            
            const m1 = dvRes.model1;
            const m2 = dvRes.model2;
            const m3 = dvRes.model3;

            // 1단계
            text += `**1단계 (독립변수 $
ightarrow$ 종속변수):**
`;
            text += `독립변수들이 종속변수에 미치는 영향을 분석한 결과, 모형의 설명력(R²)은 ${(m1.r_squared * 100).toFixed(1)}%로 나타났으며, 회귀모형은 유의하였다(F=${m1.f_value.toFixed(3)}, p=${m1.f_p_value < 0.001 ? '<.001' : m1.f_p_value.toFixed(3)}). `;
            const sigStep1 = m1.coefficients.filter((c: any) => c.p < 0.05 && c.name !== '상수');
            if (sigStep1.length > 0) {
                text += `구체적으로 ${sigStep1.map((c: any) => `'${c.name}'(β=${c.beta ? c.beta.toFixed(3) : (c.B ? c.B.toFixed(3) : 0)}, p=${c.p < 0.001 ? '<.001' : c.p.toFixed(3)})`).join(', ')}은(는) 종속변수에 유의한 영향을 미쳤다.

`;
            } else {
                text += `독립변수들 중 종속변수에 통계적으로 유의한 영향을 미치는 변수는 나타나지 않았다.
...
ightarrow$ 종속변수):**
`;
            text += `조절변수인 [${results.modParentName}]을(를) 추가로 투입한 2단계 모형의 설명력은 ${(m2.r_squared * 100).toFixed(1)}%로 나타났으며, 1단계에 비해 설명력의 변화량(ΔR²)은 ${(m2.delta_r_squared * 100).toFixed(1)}%p 증가하였다. `;
            const modCoef = m2.coefficients.find((c: any) => c.name === results.modParentName) || m2.coefficients.find((c: any) => !results.ivPayload.find((iv:any)=>iv.name===c.name) && c.name !== '상수');
            if (modCoef && modCoef.p < 0.05) {
                text += `조절변수 자체의 주효과(main effect)는 통계적으로 유의한 것으로 나타났다(p=${modCoef.p < 0.001 ? '<.001' : modCoef.p.toFixed(3)}).

`;
            } else {
                text += `조절변수 자체의 주효과는 통계적으로 유의하지 않았다.
...
`;
            ...
            text += `**3단계 (상호작용항 투입):**
`;
            text += `마지막으로 독립변수와 조절변수의 상호작용항(Interaction Term)을 추가로 투입한 결과, 모형의 전체 설명력은 ${(m3.r_squared * 100).toFixed(1)}%로 2단계 대비 ${(m3.delta_r_squared * 100).toFixed(1)}%p 변화하였다(F=${m3.f_value.toFixed(3)}, p=${m3.f_p_value < 0.001 ? '<.001' : m3.f_p_value.toFixed(3)}). `;
            
            const intCoef = m3.coefficients.find((c: any) => c.name.includes('X') || c.name.includes('x'));
            if (intCoef && intCoef.p < 0.05) {
                text += `가장 중요한 조절효과 판단 기준인 상호작용항의 통계적 유의성을 살펴본 결과, 상호작용항은 종속변수에 유의한 영향(β=${intCoef.beta ? intCoef.beta.toFixed(3) : (intCoef.B ? intCoef.B.toFixed(3) : 0)}, p=${intCoef.p < 0.001 ? '<.001' : intCoef.p.toFixed(3)})을 미치는 것으로 확인되었다. `;
                text += `**따라서 [${results.modParentName}]의 조절효과가 성립하며 가설은 채택되었다.**

`;
            } else {
                text += `그러나 조절효과 판단의 기준이 되는 상호작용항이 통계적으로 유의하지 않은 것(p=${intCoef ? intCoef.p.toFixed(3) : '>0.05'})으로 나타나 조절효과가 성립하지 않았다. `;
                text += `**따라서 가설은 기각되었다.**
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, width: '100%', minHeight: '700px', minWidth: '1000px', padding: '24px' }}>
            ...
            <div className="glass-panel" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 24px', marginBottom: '24px', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                    <h2 className="text-h3" style={{ margin: 0 }}>조절효과 분석</h2>
            ...
            {loading ? (
                ...
                results.results.forEach((dvRes: any) => {
                    ...
                    hypotheses.push({
                        id: `${hLabel}-${subHIdx++}`,
                        desc: `'${results.modParentName}'은(는) '${results.ivParentName}'와(과) '${dvRes.dv_name}' 간의 관계를 조절할 것이다.`,
                        isParent: false,
                        status: status
                    });
                });
                ...
                hypotheses.unshift({
                    id: hLabel,
                    desc: `${results.modParentName}은(는) ${results.ivParentName}과(와) ${results.dvParentName} 사이의 관계를 조절할 것이다.`,
                    isParent: true,
                    status: mainStatus
                });
                ...
                <div style={{ flex: 1, display: 'flex', gap: '24px', overflow: 'hidden' }}>
                    ...
                    <div style={{flex: '1.4', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '24px', paddingRight: '4px'}}>
                        ...
                        <ModerationTable results={results} onExport={() => handleExportMediation(results.results[0], [results.results[0].model1, results.results[0].model2, results.results[0].model3])} isExporting={isExporting2} />
                    </div>
